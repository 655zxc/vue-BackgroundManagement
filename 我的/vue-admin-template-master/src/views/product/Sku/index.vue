...
export default {
    ...
    methods:{
      ...
      //上架
      async sale(row){
        let result = await this.$API.sku.reqSale(row.row.id)
        if(result.code == 200){
          this.$message({
          message: '商品上架成功',
          type: 'success'
      ...
      //下架
      async cancel(row){
        let result = await this.$API.sku.reqCancel(row.row.id)
        if(result.code == 200){
          this.$message({
          message: '商品下架成功',
          type: 'success'
      ...
      async getSkuInfo(row){
        this.show = true
        let result = await this.$API.sku.reqSkuById(row.row.id)
